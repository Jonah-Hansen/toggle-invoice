...
def getDueDate(today, terms):
    ...
    if terms == "EOM":
        due = due.replace(
            ...
            year=due.year + 1 if due.month == 12 else due.year,
        ) - timedelta(days=1)
    if terms == "21MFI":
        due = due.replace(
            day=21,
...
def generate_invoice():
    ...
    c.setFont("Helvetica", 11)
    c.drawRightString(
        c._pagesize[0] - 1 * inch, 3.20 * inch, str(options["invoiceNumber"])
    )
    c.drawRightString(
        c._pagesize[0] - 1 * inch, 3.50 * inch, today.strftime("%d/%m/%Y")
    )
    c.drawRightString(
        c._pagesize[0] - 1 * inch,
        3.80 * inch,
        getDueDate(today, options["paymentTerms"]),
    )
    ...
    for work in times:
        ...
        hours = round(
            ...
            * round((sum(work["seconds"]) / 60) / options["roundingMinutes"])
            / 60,
            2,
        )
    ...
    data.append(["", "", options["taxType"], "{0:.2f}".format(taxes)])
    data.append(["", "", "TOTAL", "{0:.2f}".format(subtotal + taxes)])
    ...
    items = Table(
        data,
        colWidths=[3.75 * inch, inch, 0.75 * inch, inch],
        rowHeights=0.35 * inch,
        vAlign="MIDDLE",
    )
    style = TableStyle(
        [
            ("GRID", (0, 0), (-1, -4), 0.75, "#505050"),
            ("GRID", (-1, -4), (-1, -1), 0.75, "#505050"),
    ...
    items.drawOn(c, inch, c._pagesize[1] - (4.5 * inch) - h)
...
streetAddress_entry.insert(0, prefs["myInfo"]["address"].get("streetAddress", ""))
...
tk.Label(root, text="Client Street Address").grid(row=16, column=0, sticky=tk.E)
...
clientProvince_entry.insert(0, prefs["clientInfo"]["address"].get("province", ""))
...
clientCountry_entry.insert(0, prefs["clientInfo"]["address"].get("country", ""))
...
clientPostalCode_entry.insert(0, prefs["clientInfo"]["address"].get("postalCode", ""))

# Invoice Options
tk.Label(root, text="Invoice Options", font=("Helvetica", 14, "bold")).grid(
    row=21, columnspan=2, pady=10
)
...
payDays_entry.insert(0, ",".join(map(str, prefs["options"].get("payDays", []))))

tk.Label(root, text="taxes", font=("Helvetica", 12, "bold")).grid(
    row=27, columnspan=1, pady=2
)
